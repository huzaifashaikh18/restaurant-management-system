...
const getReservationForm = (req, res) => {
    res.render('reservations/new', { title: 'Reserve a Table' });
};
...
// GET /reservations/myReservations — logged-in user's own reservations
const getMyReservations = async (req, res) => {
    try {
        const reservations = await Reservation.find({ user: req.session.user._id }).sort({ date: 1 });
        res.render('reservations/myReservations', { title: 'My Reservations', reservations });
